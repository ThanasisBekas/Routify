...
- Separate API modules in `src/api/`: `authApi.ts`, `routesApi.ts`, `filtersApi.ts`, `usersApi.ts`, `tenantsApi.ts`, `auditApi.ts`, `certVaultApi.ts`, `gatewayApi.ts`, `aiApi.ts`, `apiKeysApi.ts`, `webhooksApi.ts`, `rolesApi.ts`.
