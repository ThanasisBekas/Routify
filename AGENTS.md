...
- `gr.routify.common.domain.UserRole` / `RouteStatus` — additional domain enums in the same package. `UserRole` values: `SUPER_ADMIN`, `TENANT_ADMIN`, `VIEWER`, `OPERATOR`. `RouteStatus` values: `DRAFT`, `ACTIVE`, `DISABLED`, `ARCHIVED` (lifecycle: `DRAFT` → `ACTIVE` ↔ `DISABLED` → `ARCHIVED`).
...
- `gr.routify.common.event.RequestTelemetryEvent` / `AiFilterDecisionEvent` / `AiModificationDecisionEvent` — event payloads published to their respective Kafka topics (telemetry, AI filter decisions, AI modification events).
