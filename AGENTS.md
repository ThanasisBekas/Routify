...
- `event.io.routify.common.CommandEvent` — **sealed interface** (Java 21) representing every write command sent over Kafka. All concrete command types are `record`s that implement it (e.g. `CommandEvent.CreateRoute`, `CommandEvent.UpdateFilter`, `CommandEvent.PromoteRoute`, `CommandEvent.DeployCanary`). Deserialise with `objectMapper.readValue(json, CommandEvent.class)` and use a `switch` expression on the sealed type. The `commandId` field is the idempotency key. `CommandEvent.Unknown` is the fallback for unrecognised `"type"` discriminators.
