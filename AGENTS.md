...
- **AI audit** (on `EXCHANGE_AUDIT_SERVICE`): `QUEUE_AUDIT_AI_FILTER_STATS` (`audit.ai-filter.stats`) and `QUEUE_AUDIT_AI_FILTER_QUERY` (`audit.ai-filter.query`) — serve AI filter decision analytics to admin-api. `AiFilterStatsQuery` accepts optional `promptVersionId` field for A/B version-filtered stats.
- **AI prompt versions** (on `EXCHANGE_AUDIT_SERVICE`): `QUEUE_AI_PROMPT_VERSIONS_QUERY` (`ai-prompt.versions.query`) / `QUEUE_AI_PROMPT_VERSIONS_GET` (`ai-prompt.versions.get`) / `QUEUE_AI_PROMPT_VERSIONS_SAVE` (`ai-prompt.versions.save`) — prompt version CRUD for AI filter policies. `QUEUE_AI_DECISION_LABEL` (`ai-decision.label`) — operator ground-truth labelling (CORRECT/INCORRECT/UNCLEAR) for accuracy scoring. Admin-api hits `/api/v1/admin/ai-filter/{filterId}/versions` and `POST /api/v1/admin/ai-filter/decisions/{evaluationId}/label`.
...
- **Time-series analytics** (on `EXCHANGE_AUDIT_SERVICE`): `QUEUE_AUDIT_TIME_SERIES` (`audit.time-series`) — time-bucketed request metrics using SQL `date_trunc(granularity, requested_at)` for the GraphQL Analytics API. Supports MINUTE, HOUR, DAY, WEEK granularity. Request/response types: `QueryRequest.TimeSeriesQuery` / `QueryResponse.TimeSeriesResult`.
- **Alerting engine** (on `EXCHANGE_AUDIT_SERVICE`): `QUEUE_ALERT_RULES_QUERY` (`alert-rules.query`) / `QUEUE_ALERT_RULES_GET` (`alert-rules.get`) / `QUEUE_ALERT_EVENTS_QUERY` (`alert-events.query`) / `QUEUE_ALERT_RULES_COMMAND` (`alert-rules.command`) — alert rule CRUD and state transition history. All operations are sync RPC. `AlertEvaluationScheduler` evaluates rules every 60s (configurable via `routify.alerts.evaluation-interval-ms`). State machine: OK → PENDING → FIRING → OK. Fires `ALERT_FIRED` / `ALERT_RESOLVED` events to `AUDIT_EVENTS` Kafka topic for webhook delivery. Admin-api hits `/api/v1/admin/alerts`. Dashboard module: `src/modules/alerts/`.
...
- Spring Boot **4.0.5**, Spring Cloud **2025.1.1**, Spring AI **2.0.0-M4**, JJWT **0.13.0**, Resilience4j **2.2.0**, MapStruct **1.6.3**.
...
- Actuator management ports are `9080`–`9087` (app port + 1000). Prometheus metrics scraped there.
...
- Metrics are registered in `observability.io.routify.common.RoutifyMetrics` under the `routify.*` namespace (auto-configured via `RoutifyMetricsAutoConfiguration`). ACME lifecycle metrics: `routify.cert.acme.renewals` (successful auto-renewals counter), `routify.cert.acme.failures` (failed ACME operations counter). Quota metric: `routify.gateway.requests.quota_exceeded` (requests rejected due to monthly tenant quota exceeded). Cluster metric: `routify.gateway.cluster.config-version` (gauge, local config version counter per gateway instance). Canary metrics: `routify.canary.deployments` (counter, incremented on each canary deployment), `routify.canary.rollbacks` (counter, incremented on manual + auto rollbacks). Alerting metrics: `routify.alerts.evaluation` (timer, time spent evaluating alert rules per cycle), `routify.alerts.fired` (counter, alert rules transitioned to FIRING). Response cache metric: `routify.filter.response_cache.purges` (counter, cache purge operations triggered via `CommandEvent.PurgeCacheRoute`). Webhook cleanup metric: `routify.webhooks.delivery.cleanup` (counter, expired webhook delivery records purged by nightly scheduler).
...
- All `routify-admin-api` REST endpoints use the `/api/v1/admin/` prefix (e.g. `/api/v1/admin/routes`, `/api/v1/admin/gateway`, `/api/v1/admin/audit/replay`, `/api/v1/admin/roles`). Auth endpoints use `/api/v1/auth/`. Gateway config endpoints live in a separate package (`gateway.controller.io.routify.admin.GatewayConfigController`). `AdminDashboardController` serves aggregated dashboard stats (`GET /api/v1/admin/stats`, `GET /api/v1/admin/dashboard/gateway-status`) and an SSE event stream (`GET /api/v1/admin/events`) via `DashboardStatsService` and `DashboardEventBroadcaster` (`sse/` package). WebSocket/STOMP support is in `ws/` package: `WebSocketController` (STOMP message handling) and `WebSocketEventBroadcaster` (pushes domain events, metrics, and audit entries to STOMP topics). `FleetHealthScheduler` (`service/` package, `@Scheduled(fixedRate=60_000)`) monitors gateway fleet health and publishes `GATEWAY_CONFIG_DRIFT` audit events when any instance's config version falls behind. Each downstream messaging client in admin-api (e.g. `RouteServiceClient`, `RouteFilterMessagingClient`, `IdentityMessagingClient`, `AuditMessagingClient`, `CertVaultMessagingClient`, `AiMessagingClient`) is wrapped with a **Resilience4j circuit breaker** named after the service (e.g. `"route-service"`, `"identity-service"`) with a 5 s time limiter.
- **GraphQL Analytics API** (Initiative 13): `routify-admin-api` exposes a GraphQL endpoint at `/api/v1/admin/graphql` alongside existing REST endpoints. Schema at `src/main/resources/graphql/schema.graphqls`. Queries: `routeAnalytics` (per-route time-bucketed metrics), `tenantUsage` (quota usage + daily trend), `aiFilterAnalytics` (AI decision stats), `certExpiryReport` (cert vault expiry), `auditTimeline` (event log). Custom scalars: `DateTime` (ISO-8601 Instant), `Date` (ISO-8601 LocalDate), `Long`. Controller: `graphql.io.routify.admin.AnalyticsGraphQLController`. Security: `GraphQLSecurityInterceptor` enforces JWT auth and tenant isolation. Query depth limited to 5, complexity limited to 100 via `MaxQueryDepthInstrumentation` / `MaxQueryComplexityInstrumentation`. GraphiQL playground enabled in dev profile at `/api/v1/admin/graphiql`. Dashboard client: `src/api/graphqlClient.ts` (uses `apiClient` Axios instance). Time-series backend: `QUEUE_AUDIT_TIME_SERIES` (`audit.time-series`) — `date_trunc`-based SQL aggregation in audit-service `RequestLogRepository.getTimeSeriesMetrics()`.
...
- **Canary Routing & Traffic Splitting (Initiative 14)**: Weighted traffic splitting between route versions for progressive delivery. Domain model: `route` table has `traffic_weight` (0–100, default 100), `canary_route_id` (nullable FK), `canary_auto_rollback_threshold` (error rate %). Flyway migration: `V8__canary_routing.sql`. Command events: `CommandEvent.DeployCanary`, `PromoteCanary`, `RollbackCanary`, `AdjustCanaryWeight` — all sent on `ROUTE_COMMANDS` topic. Domain events: `DomainEvent.CanaryDeployed`, `CanaryPromoted`, `CanaryRolledBack`. Route-service `RouteService` implements `deployCanary()` (creates sibling route with `-canary` suffix, copies filter chain, sets `canaryPrimaryRouteId` in canary's `extraConfig` for gateway weight grouping), `promoteCanary()` (copies canary upstream → primary, archives canary), `rollbackCanary()` (archives canary, restores primary to 100%), `adjustCanaryWeight()`. Gateway: `RouteDefinitionBuilder` adds a `Weight` predicate when `trafficWeight < 100` — both primary and canary share the same weight group (`canary-{primaryRouteId}`) so Spring Cloud Gateway's `WeightRoutePredicateFactory` handles probabilistic selection. Admin-api: `CanaryMonitorService` (`@Scheduled(fixedRate=30_000)`) monitors canary error rates via audit-service health data; 3 consecutive breaches trigger auto-rollback via `CommandEvent.RollbackCanary`. Endpoints on `AdminRoutesController`: `POST /{id}/canary` (deploy), `GET /{id}/canary/status` (health + error rate comparison), `POST /{id}/canary/promote`, `POST /{id}/canary/rollback`, `PUT /{id}/canary/weight`. Dashboard: `CanaryDeployModal.tsx` (upstream URI, weight slider 5–50%, threshold), `CanaryStatusPanel.tsx` (dual progress bars, error rate comparison, promote/rollback buttons, weight adjustment), `TrafficWeightSlider.tsx`. MSW mock handlers in `src/mocks/handlers/routes.ts`. TypeScript types: `DeployCanaryRequest`, `AdjustCanaryWeightRequest`, `CanaryStatusResponse` in `src/types/index.ts`.
...
- `event.io.routify.common.CommandEvent` — **sealed interface** (Java 21) representing every write command sent over Kafka. All concrete command types are `record`s that implement it (e.g. `CommandEvent.CreateRoute`, `CommandEvent.UpdateFilter`, `CommandEvent.PromoteRoute`, `CommandEvent.DeployCanary`, `CommandEvent.PurgeCacheRoute`, `CommandEvent.ForceCircuitBreaker`). Deserialise with `objectMapper.readValue(json, CommandEvent.class)` and use a `switch` expression on the sealed type. The `commandId` field is the idempotency key. `CommandEvent.Unknown` is the fallback for unrecognised `"type"` discriminators.
...
- `domain.io.routify.common.FilterType` — enum of all gateway filter types (keep in sync with TypeScript `FilterType` union in dashboard and every `*GatewayFilterFactory` in the gateway). Active types include `AUTH_*` (`AUTH_API_KEY`, `AUTH_BASIC`, `AUTH_JWT`, `AUTH_MTLS`, `AUTH_OAUTH2`, `AUTH_CLIENT_ID`, `AUTH_CERT_VAULT`), `DOWNSTREAM_BASIC_AUTH`, `DOWNSTREAM_BEARER_CC`, `OAUTH2_TOKEN_RELAY`, `RATE_LIMIT_FIXED_WINDOW`, `RATE_LIMIT_SLIDING_WINDOW`, `REQUEST_HEADER_MODIFY`, `RESPONSE_HEADER_MODIFY`, `RESPONSE_HEADER_REWRITE`, `BODY_JOLT_TRANSFORM`, `VALIDATE_JSON_SCHEMA`, `REQUEST_SIZE_LIMIT`, `GRAPHQL_DEPTH_LIMIT`, `RESPONSE_CACHE`, `REQUEST_DECOMPRESS`, `IDEMPOTENCY_KEY`, `TIMEOUT`, `CIRCUIT_BREAKER_V2`, `RETRY_V2`, `CONDITIONAL_ROUTE`, `USER_ID_PAYLOAD_ROUTING`, `GEO_ROUTE`, `IP_ACCESS_CONTROL`, `CERT_ROTATION`, `CERT_VAULT_EXPIRY_CHECK`, `API_VERSIONING`, `CORRELATION_ID`, `REQUEST_LOGGER`, `TENANT_CONTEXT`, `SECURITY_HEADERS`, `CUSTOM_METRIC`, `BODY_SIZE_METRIC`, `WEBHOOK_NOTIFY`, `MOCK_RESPONSE`, `CUSTOM_SPEL`, `AI_FILTER`, `AI_MODIFIER`. Several legacy values are `@Deprecated` (no factory implementation — kept for DB compatibility only; `RouteDefinitionBuilder` logs a warning and skips them): `AUTH_NONE`, `RATE_LIMIT_TOKEN_BUCKET`, `PATH_REWRITE`, `PATH_STRIP_PREFIX`, `PATH_ADD_PREFIX`, `QUERY_PARAM_MODIFY`, `BODY_JSONATA_TRANSFORM`, `BODY_SPEL_TRANSFORM`, `VALIDATE_REGEX`, `VALIDATE_SIZE`, `CIRCUIT_BREAKER`, `RETRY`.
- `domain.io.routify.common.TenantPlan` — enum (`FREE`, `STARTER`, `PRO`, `ENTERPRISE`) encoding `maxRoutes`, `maxFilters`, `monthlyRequestQuota`, and `maxRequestBodySize` (bytes) quotas. Per-plan body size limits: FREE=5 MB, STARTER=10 MB, PRO=50 MB, ENTERPRISE=500 MB. **Quotas are enforced at two layers**: (1) route-service checks `currentRouteCount < plan.maxRoutes()` and `currentFilterCount < plan.maxFilters()` before executing `CreateRoute`/`CreateFilter` commands (throws `RoutifyException.QuotaExceeded`); (2) gateway's `TenantContextGatewayFilterFactory` atomically increments a Redis monthly counter (`RedisKeys.QUOTA_PREFIX + tenantId + ":" + YearMonth`) and returns HTTP 429 with `Retry-After` when `count > plan.monthlyRequestQuota()`. Both layers maintain a local `ConcurrentHashMap<UUID, TenantPlan>` plan cache populated from `TENANT_EVENTS` Kafka topic. ENTERPRISE plan (Integer.MAX_VALUE) bypasses all quota checks. Gateway quota can be disabled via `routify.gateway.quota.enabled: false`.
- `domain.io.routify.common.WebhookEventType` — enum of all platform event types that can trigger webhook notifications (e.g. `ROUTE_ACTIVATED`, `CERT_EXPIRING`, `AI_FILTER_BLOCKED`, `QUOTA_WARNING`, `QUOTA_EXCEEDED`, `GATEWAY_CONFIG_DRIFT`, `CANARY_DEPLOYED`, `CANARY_PROMOTED`, `CANARY_ROLLBACK`, `ALERT_FIRED`, `ALERT_RESOLVED`). Keep in sync with TypeScript `WebhookEventType` union in the dashboard `src/types/index.ts`.
...
- `domain.io.routify.common.AlertMetric` — enum of metric types available for alert rules: `ERROR_RATE`, `P99_LATENCY`, `DLQ_DEPTH`, `CERT_EXPIRY_DAYS`, `QUOTA_USAGE`, `SLO_BUDGET`, `REQUEST_VOLUME`, `AUTH_FAILURE_RATE`. Keep in sync with TypeScript `AlertMetric` union in `src/types/index.ts`.
...
- Gateway filter factories are organised into sub-packages under `filter.io.routify.gateway`: `auth/` (OAuth2TokenRelayGatewayFilterFactory), `devex/` (MockResponseGatewayFilterFactory), `integration/` (WebhookNotifyGatewayFilterFactory), `observability/` (BodySizeMetricGatewayFilterFactory), `performance/` (ResponseCacheGatewayFilterFactory, RequestDecompressGatewayFilterFactory), `ratelimit/` (RateLimitKeyResolver), `reliability/` (IdempotencyKeyGatewayFilterFactory), `resilience/` (CircuitBreakerV2GatewayFilterFactory, RetryV2GatewayFilterFactory, CircuitBreakerCommandKafkaConsumer), `routing/` (GeoRouteGatewayFilterFactory), `security/` (IpAccessControlGatewayFilterFactory, CidrMatcher), `shared/` (GatewayProblemResponse — RFC 9457 error helper), `validation/` (GraphQLDepthLimitGatewayFilterFactory, RequestSizeLimitGatewayFilterFactory). Top-level filter factories (e.g. `AiGatewayFilterFactory`, `JwtAuthGatewayFilterFactory`) remain directly under `filter/`.
- `routing.io.routify.gateway.CachePurgeKafkaConsumer` — listens for `CommandEvent.PurgeCacheRoute` on the route commands topic and evicts Redis cache keys for the target route. Uses a shared consumer group so every gateway instance processes the purge.
- `filter.resilience.io.routify.gateway.CircuitBreakerCommandKafkaConsumer` — listens for `CommandEvent.ForceCircuitBreaker` to allow operators to manually open/close/half-open per-route circuit breakers from the dashboard.
- `net.io.routify.gateway` — per-route upstream HTTP client infrastructure: `WebClientFactory` builds non-blocking `WebClient` instances, `WebClientRegistry` caches them keyed by route ID, `WebClientRegistryRefreshListener` evicts on route reload. `HttpClientProperties` / `ProxyProperties` configure connection pooling and proxy settings.
...
- `src/modules/ai/` — AI filter and AI modifier stats pages, AI Policy Playground (`AiPlaygroundPage.tsx` — interactive prompt editor with version history, test request builder, verdict display), and Version Comparison page (`VersionComparisonPage.tsx` — side-by-side prompt version A/B metrics). Sub-components in `components/`: `PromptEditor.tsx`, `TestRequestBuilder.tsx`, `VerdictDisplay.tsx`. `src/modules/api-keys/` — API key lifecycle management (create, revoke, rotate). `src/modules/audit/` — audit log viewer. `src/modules/auth/` — login, password change, protected route guard. `src/modules/certificates/` — cert vault + cert groups management. `src/modules/filters/` — filter definition and configuration. `src/modules/gateway/` — gateway health dashboard v2 with 13 tabs: Overview, Routes Health (latency heatmap), SLOs (error budget tracking), Circuit Breakers (live WS), Fleet (multi-gateway cluster instances and config versions), CORS, Security Headers, Rate Limiting, Resilience, Auth Providers, Networking, Tenant Isolation, Global Filters. New tabs added in `tabs/RoutesHealthTab.tsx`, `tabs/SloTab.tsx`, `tabs/CircuitBreakersTab.tsx`, `tabs/FleetTab.tsx`. Shared `TraceLink` component in `components/TraceLink.tsx` links to Grafana Tempo. `components/InstanceCard.tsx` renders per-instance fleet status cards. `components/GatewayPrimitives.tsx` provides reusable gateway dashboard UI primitives. `src/modules/gitops/` — GitOps reconciliation agent dashboard (status, history, sync trigger). `src/modules/roles/` — role definitions and permission management (granular RBAC). `src/modules/routes/` — route management, canary deployment (CanaryDeployModal, CanaryStatusPanel, TrafficWeightSlider). `src/modules/settings/` — platform settings. `src/modules/users/` — user management. `src/modules/webhooks/` — webhook subscription management (create, delete, test ping, delivery log). `src/modules/workspaces/` — workspace (tenant) management with usage analytics (UsageOverview progress bars, UsageTrendChart daily request chart). `src/modules/workflow-builder/` — visual route topology editor (shares `@xyflow/react` with `src/modules/routes/`). `src/modules/alerts/` — platform alerting engine dashboard (AlertsPage rule list with state indicators, AlertRuleFormModal create/edit form, AlertHistoryTimeline state transition history, mute/unmute controls, pre-built template buttons).
- Separate API modules in `src/api/`: `authApi.ts`, `routesApi.ts`, `filtersApi.ts`, `usersApi.ts`, `tenantsApi.ts`, `auditApi.ts`, `certVaultApi.ts`, `gatewayApi.ts`, `aiApi.ts`, `apiKeysApi.ts`, `webhooksApi.ts`, `rolesApi.ts`, `exportImportApi.ts`, `gitopsApi.ts`, `alertsApi.ts`.
