...
| # | Task | Module | Detail |
|---|------|--------|--------|
| 2.1 | Implement API Key validation | `routify-api-gateway` | `ApiKeyAuthGatewayFilterFactory` (line 45) has `// TODO: Validate against Redis/identity service in production`. Currently any non-empty key passes through. Implement reactive Redis hash lookup (`HGET routify:apikeys:<key>`) and inject resolved identity headers. |
| 2.2 | Add method-level authorization | `routify-admin-api` | Only `AdminTenantsController` has `@Secured` guards (2 endpoints). `AdminRoutesController`, `AdminUsersController`, `AdminFiltersController`, `AdminCertificatesController`, `AdminCertGroupsController`, `AdminAuditController`, `AdminReplayController` have **no role checks** — any authenticated user can create/delete anything. Add `@PreAuthorize` annotations per controller matching `UserRole` enum (`SUPER_ADMIN`, `TENANT_ADMIN`, `OPERATOR`, `VIEWER`). |
| 2.3 | Add nginx security headers | `routify-dashboard` | `nginx.conf` is missing: `Content-Security-Policy`, `Strict-Transport-Security` (HSTS), `Permissions-Policy`. Add to the `server` block. Also add `gzip_min_length 256;` to skip tiny responses. |
| 2.4 | Protect Redis with a password | `docker-compose.yml` | Redis runs with no password (`redis-server` with no `--requirepass`). Add `--requirepass ${REDIS_PASS}` and configure `spring.data.redis.password` in all services that use Redis (route-service, admin-api, ai-service, identity-service, api-gateway). Add `REDIS_PASS` to `environments/README.md`. |
| 2.5 | Add `ai-service` circuit breaker in admin-api | `routify-admin-api` | `AiMessagingClient` calls `EXCHANGE_AI_SERVICE` via RabbitMQ but admin-api's `application.yml` has no Resilience4j circuit breaker entry for `ai-service`. A stuck LLM call blocks the admin-api thread pool. Add `ai-service` CB config alongside the existing 4. |
...
| # | Task | Module | Detail |
|---|------|--------|--------|
| 3.1 | Add distributed tracing | All services | No `micrometer-tracing` dependency exists. Add `micrometer-tracing-bridge-otel` + `opentelemetry-exporter-otlp` to parent POM. Add Grafana Tempo container to `docker-compose.yml` (integrates with existing Grafana). Propagate trace context through Kafka record headers and RabbitMQ `MessageProperties`. |
| 3.2 | Enrich MDC with `tenantId` + `userId` | All services | Currently only `correlationId` is in MDC (set in `KafkaDlqErrorHandlerFactory` and the log pattern). Add `tenantId` and `userId` from `SecurityContext` in every JWT filter and Kafka consumer so log aggregation can filter by tenant. |
| 3.3 | Add platform alert rules | `docker/prometheus/alert_rules.yml` | Existing rules only cover AI filter/modifier. Add rules for: Kafka consumer lag (`kafka_consumer_lag > 1000 for 5m`), DLQ record growth, outbox backlog (`routify.outbox.pending > 100`), RabbitMQ reply timeouts, HikariCP active connections > 80%, circuit breaker state changes on admin-api. |
| 3.4 | Expand `RoutifyMetrics` | `routify-common` | Current metrics are gateway-centric (request counters, route gauges). Add: `routify.outbox.pending` gauge, `routify.rpc.latency` timer (per exchange), `routify.dlq.events` counter (per DLQ topic), `routify.cert.expiry.days` gauge. |
| 3.5 | Add missing DLQ constant | `routify-common` | `KafkaTopics` defines `DLQ_AI_MODIFICATION_EVENTS` but **not** `DLQ_AI_FILTER_DECISIONS`. If the audit-service consumer for `AI_FILTER_DECISIONS` fails, records go to an untracked DLQ topic. Add `DLQ_AI_FILTER_DECISIONS = AI_FILTER_DECISIONS + ".DLQ"` and register it in the DLQ consumer. |
...
| # | Task | Module | Detail |
|---|------|--------|--------|
| 4.1 | Add application-level caching | Multiple | Only `routify-ai-service` uses `@EnableCaching`. Add Caffeine caches for: route-service gateway snapshot (invalidated on outbox publish), identity-service user/tenant lookups, admin-api tenant list for login dropdown. |
| 4.2 | Tune audit-service HikariCP | `routify-audit-service` | `maximum-pool-size: 10` with `concurrency: 3` Kafka consumers doing batch inserts. Under load, 3 concurrent consumers × batch writes can saturate the pool. Increase to `15`, add `connection-timeout: 10000`, add `pool-name: audit-pool` for Prometheus monitoring. |
| 4.3 | Add frontend code-splitting | `routify-dashboard` | `App.tsx` eagerly imports all 11 page components. No `React.lazy` or `Suspense` usage. Lazy-load heavy modules (`WorkflowBuilderPage`, `AuditPage`, `GatewayPage`, `CertVaultPage`) to reduce initial bundle size. |
| 4.4 | Optimize outbox polling | `routify-route-service` | `OutboxPoller` polls every 250ms. Most polls return zero rows. Add PostgreSQL `LISTEN/NOTIFY` trigger on `outbox_events` insert to wake the poller on demand, reducing empty database round-trips. |
| 4.5 | Document audit-service CB threshold asymmetry | `routify-admin-api` | `audit-service` circuit breaker has `failure-rate-threshold: 60` while all others have `50`. If intentional (audit is less critical), add a YAML comment. Otherwise align to `50`. |
...
| # | Task | Module | Detail |
|---|------|--------|--------|
| 5.1 | Add shared UI primitives | `routify-dashboard` | `src/components/ui/` contains only `Select.tsx`. Extract reusable `Input`, `Button`, `Modal`, `Badge`, `Table`, `Card` components to enforce consistent styling and reduce duplication across modules. |
| 5.2 | Add ARIA labels and accessibility | `routify-dashboard` | Only **5** `aria-*` attributes exist across the entire dashboard. Add `eslint-plugin-jsx-a11y` to `eslint.config.js` and systematically fix violations: form labels, button labels, modal roles, toast announcements, navigation landmarks. |
| 5.3 | Add per-page document titles | `routify-dashboard` | Document title is static "Routify" from `index.html`. Add a `useDocumentTitle` hook or use React Router's `handle` to set `<title>` per page (e.g. "Routes — Routify", "Audit — Routify"). |
| 5.4 | Add React `ErrorBoundary` per module | `routify-dashboard` | `ErrorBoundary.tsx` exists in `src/components/` but is not wrapped around individual module routes. A crash in `WorkflowBuilderPage` takes down the entire app. Wrap each `<Route>` in `App.tsx` with an error boundary that shows a retry button. |
| 5.5 | Add `prettier` for consistent formatting | `routify-dashboard` | No `prettier` config exists. Add `.prettierrc` and `prettier --check` to the `lint` script so formatting is enforced in CI. |
