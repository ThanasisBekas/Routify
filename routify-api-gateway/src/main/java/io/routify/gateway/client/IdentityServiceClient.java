...
/**
 * RabbitMQ RPC client for calling routify-identity-service from routify-api-gateway.
 *
 * <p>Used for:
 * <ul>
 *   <li>Bulk TenantPlanCache warmup at startup</li>
 *   <li>On-demand single-tenant plan fetch on cache miss (async, off the Netty event loop)</li>
 * </ul>
 */
@Slf4j
@Component
public class IdentityServiceClient extends AmqpServiceClientSupport {
    ...
    public IdentityServiceClient(RabbitTemplate rabbitTemplate, ObjectMapper objectMapper) {
        super(rabbitTemplate, objectMapper, RabbitTopology.EXCHANGE_IDENTITY_SERVICE, "api-gateway");
    }
     ...
     * <strong>Blocking</strong> — must be called from a non-event-loop thread
     * (e.g. {@code Schedulers.boundedElastic()}).
     *
