...
export const aiApi = {
  ...
  testPolicy: (req: { policyDescription: string; sampleRequest: AiModificationTestRequest['sampleRequest'] }) =>
    apiClient.post<{ action: string; reason: string; confidence: number }>(`${AI_BASE}-filter/test-policy`, req)
      .then(r => r.data),
  ...
  testModification: (req: AiModificationTestRequest) =>
    apiClient.post<AiModificationTestResult>(`${AI_BASE}-modifier/test-modification`, req)
      .then(r => r.data),
  ...
  getAiFilterStats: (routeId: string, from?: string, to?: string) =>
    apiClient.get<AiFilterStats>('/api/v1/admin/audit/ai-filter/stats', { params: { routeId, from, to } })
      .then(r => r.data),
  ...
  listAiFilterDecisions: (routeId: string, params?: { page?: number; size?: number; action?: string }) =>
    apiClient.get<Page<AiFilterDecisionEntry>>('/api/v1/admin/audit/ai-filter/decisions', {
  ...
  getAiModifierStats: (routeId: string, from?: string, to?: string) =>
    apiClient.get<AiModifierStats>('/api/v1/admin/audit/ai-modifier/stats', { params: { routeId, from, to } })
      .then(r => r.data),
  ...
  listAiModifierDecisions: (routeId: string, params?: { page?: number; size?: number }) =>
    apiClient.get<Page<AiModifierDecisionEntry>>('/api/v1/admin/audit/ai-modifier/decisions', {
      params: { routeId, ...params },
    }).then(r => r.data),
}
