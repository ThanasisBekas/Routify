...
export const webhooksApi = {
  ...
  create: (data: CreateWebhookRequest) =>
    apiClient.post<AsyncAcknowledgement>(BASE, data).then((r) => r.data),

  update: (id: string, data: UpdateWebhookRequest) =>
    apiClient.put<AsyncAcknowledgement>(`${BASE}/${id}`, data).then((r) => r.data),

  delete: (id: string) =>
    apiClient.delete<AsyncAcknowledgement>(`${BASE}/${id}`).then((r) => r.data),

  /** Send a test ping to the webhook — returns result inline. */
  testPing: (id: string) =>
    apiClient.post<WebhookTestResult>(`${BASE}/${id}/test`).then((r) => r.data),
