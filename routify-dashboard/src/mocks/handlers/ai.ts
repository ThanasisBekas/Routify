...
import type {
  AiFilterStats, AiFilterDecisionEntry,
  AiModifierStats, AiModifierDecisionEntry,
  AiModificationTestRequest, AiModificationTestResult,
} from '../../types'
...
function makeModifierDecisions(routeId: string): AiModifierDecisionEntry[] {
  return [
    { mutationId: `mut-${routeId}-1`, routeId, routeName: 'Payments API', mutationApplied: true,  mutationType: 'PII_SCRUB',    reason: 'Scrubbed email and phone from request body.', headersModified: [], cached: false, latencyMs: 340, method: 'POST', path: '/api/v1/payments/charge', evaluatedAt: new Date(Date.now() - 60000).toISOString() },
    { mutationId: `mut-${routeId}-2`, routeId, routeName: 'Payments API', mutationApplied: false, mutationType: 'PASSTHROUGH',  reason: 'No PII detected in request body.',            headersModified: [], cached: true,  latencyMs: 3,   method: 'GET',  path: '/api/v1/payments/status', evaluatedAt: new Date(Date.now() - 120000).toISOString() },
    { mutationId: `mut-${routeId}-3`, routeId, routeName: 'Payments API', mutationApplied: true,  mutationType: 'HEADER_REWRITE', reason: 'Rewrote User-Agent header.',                 headersModified: ['User-Agent', 'X-Client-Id'], cached: false, latencyMs: 215, method: 'POST', path: '/api/v1/payments/refund', evaluatedAt: new Date(Date.now() - 180000).toISOString() },
  ]
}
...
export const aiHandlers = [
  ...
  http.post(`${AI_BASE}-filter/test-policy`, async ({ request }) => {
    ...
    const isBlock = body.policyDescription.toLowerCase().includes('block') ||
  ...
  http.post(`${AI_BASE}-modifier/test-modification`, async ({ request }) => {
    ...
    const isPii = body.modificationPrompt.toLowerCase().includes('pii') ||
                  body.modificationPrompt.toLowerCase().includes('scrub')
