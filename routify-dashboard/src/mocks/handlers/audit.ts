...
export const auditHandlers = [
  ...
  http.get(`${BASE}/replay/pending`, async ({ request }) => {
    ...
    const page = parseInt(url.searchParams.get('page') ?? '0', 10)
    const size = parseInt(url.searchParams.get('size') ?? '20', 10)
    const items = failedRequests.filter(
      (r) => !r.replayStatus || r.replayStatus === 'PENDING' || r.replayStatus === 'FAILED',
    )
    return HttpResponse.json(buildPage(items, page, size))
  }),
  ...
  http.post(`${BASE}/replay/:id`, async ({ params }) => {
    ...
    failedRequests[idx] = {
      ...
      replayCount: (failedRequests[idx].replayCount ?? 0) + 1,
      replayedAt: new Date().toISOString(),
      replayResponseStatus: 200,
    }
    return HttpResponse.json({
      requestLogId: params.id,
      outcome: 'SUCCEEDED',
      responseStatus: 200,
