...
export const tenantHandlers = [
  ...
  http.post('/api/v1/admin/tenants', async ({ request }) => {
    ...
    if (dup) return HttpResponse.json({ status: 409, detail: `Workspace slug '${body.slug}' already exists.` }, { status: 409 })
    const now    = new Date().toISOString()
    const tenant: TenantDto = {
      id: genId(), name: body.name, slug: body.slug,
      status: 'ACTIVE', plan: body.plan as TenantPlan,
      contactEmail: body.contactEmail, createdAt: now,
    }
