...
export const userHandlers = [
  ...
  http.post(BASE, async ({ request }) => {
    ...
    if (duplicate) return HttpResponse.json({ status: 409, detail: `Username '${body.username}' is already taken.` }, { status: 409 })
    const now  = new Date().toISOString()
    const tenantId = request.headers.get('X-Tenant-Id') || MOCK_TENANT_ID
    const user: UserDto = {
      id: genId(), tenantId, username: body.username,
      email: body.email, role: body.role, status: 'ACTIVE',
  ...
  http.delete(`${BASE}/:id`, async ({ params }) => {
    await delay(300)
    if (!users.has(params.id as string)) return HttpResponse.json({ status: 404, detail: 'User not found' }, { status: 404 })
  ...
  http.post(`${BASE}/:id/reset-password`, async ({ params }) => {
    ...
    users.set(user.id, updated)
    return HttpResponse.json({ success: true, message: `Password reset for ${user.username}. User must change it on next login.` })
  }),
]
