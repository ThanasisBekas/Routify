...
function ConfirmButton({
  onConfirm, disabled, isLoading, title,
  icon: Icon, hoverColor, confirmIcon: ConfirmIcon, confirmLabel,
}: {
  onConfirm: () => void
  disabled?: boolean
  isLoading?: boolean
...
function StatPill({
  ...
  return (
    <div className={cn(
      ...
      highlight === 'amber' ? 'bg-amber-500/8 border-amber-500/20' :
      highlight === 'red'   ? 'bg-red-500/8 border-red-500/20' :
    ...
    >
      ...
      <div className="flex items-start gap-3 mb-3 pr-16">
        <div className={cn(
          ...
          group.expiryHealthStatus === 'EXPIRED'       ? 'bg-red-500/10 border-red-500/20' :
          group.expiryHealthStatus === 'EXPIRING_SOON' ? 'bg-amber-500/10 border-amber-500/20' :
          'bg-violet-500/10 border-violet-500/20'
        )}>
          <Layers className={cn(
            'w-4 h-4',
            group.expiryHealthStatus === 'EXPIRED'       ? 'text-red-400' :
            group.expiryHealthStatus === 'EXPIRING_SOON' ? 'text-amber-400' :
            'text-violet-400'
          )} />
        </div>
      ...
      <div className="flex items-center gap-3 text-xs text-gray-500">
        <div className="flex items-center gap-1">
          ...
          <span>{group.memberCount} cert{group.memberCount !== 1 ? 's' : ''}</span>
        </div>
        <div className="flex items-center gap-1">
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 px-3 rounded-lg bg-white/2 border border-white/4 group hover:bg-white/3 transition-colors">
      ...
      <div className={cn(
        ...
        cert.status === 'ACTIVE'  ? 'bg-indigo-500/10 border-indigo-500/20' :
        cert.status === 'REVOKED' ? 'bg-red-500/10 border-red-500/20' :
        'bg-gray-500/10 border-gray-500/20'
      )}>
        <ShieldCheck className={cn(
          'w-3.5 h-3.5',
          cert.status === 'ACTIVE'  ? 'text-indigo-400' :
          cert.status === 'REVOKED' ? 'text-red-400' : 'text-gray-500'
        )} />
      </div>
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <button onClick={onView} className="text-sm text-white font-medium hover:text-indigo-300 transition-colors text-left truncate max-w-36">
            {cert.alias}
          </button>
          {cert.memberAlias && (
            <span className="text-[10px] font-medium text-violet-400 bg-violet-500/10 px-1.5 py-0.5 rounded shrink-0">{cert.memberAlias}</span>
          )}
          <span className={cn('text-[10px] font-medium px-1.5 py-0.5 rounded-full border shrink-0', STATUS_COLOR[cert.status])}>
            {cert.status}
          </span>
        </div>
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-4 text-center px-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 leading-relaxed">
                  Groups bundle certificates under a stable <span className="font-mono text-gray-400">logicalId</span> key.
                  The gateway always loads certificates via this key — enabling zero-downtime rotation.
                </p>
              </div>
              <div className="flex items-start gap-2 p-3 rounded-lg bg-indigo-500/5 border border-indigo-500/15 text-xs text-left">
                ...
                <span className="text-indigo-300/80">
                  Workflow: <span className="font-semibold text-indigo-300">Create group</span> → upload certs into it → gateway serves them under the group's logical ID.
                </span>
              </div>
              <button
                onClick={() => setShowCreateForm(true)}
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3 min-w-0">
                    <div className={cn(
                      ...
                      detail.expiryHealthStatus === 'EXPIRED'       ? 'bg-red-500/10 border-red-500/20' :
                      detail.expiryHealthStatus === 'EXPIRING_SOON' ? 'bg-amber-500/10 border-amber-500/20' :
                      'bg-violet-500/10 border-violet-500/20'
                    )}>
                      <Layers className={cn(
                        'w-5 h-5',
                        detail.expiryHealthStatus === 'EXPIRED'       ? 'text-red-400' :
                        detail.expiryHealthStatus === 'EXPIRING_SOON' ? 'text-amber-400' :
                        'text-violet-400'
                      )} />
                    </div>
                    <div className="min-w-0">
              ...
              {detail.members && detail.members.length > 0 && detail.status === 'ACTIVE' && (
                <div className="mt-5 p-3 rounded-lg bg-violet-500/5 border border-violet-500/10">
                  <p className="text-xs text-violet-300/70 leading-relaxed">
                    <span className="font-semibold text-violet-300">Zero-downtime rotation:</span>{' '}
                    Upload a new certificate → the gateway immediately serves it under{' '}
