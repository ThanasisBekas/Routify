...
export default function CertUploadModal({ tenantId, preselectedGroup, onClose, onSuccess }: Props) {
  ...
  const uploadMutation = useMutation({
    mutationFn: () =>
      certVaultApi.uploadCertificate(tenantId, {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setError('')
    if (!selectedGroupId) {
      setError('You must select a certificate group first')
      return
    }
    if (!form.alias.trim()) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="relative w-full max-w-2xl bg-[#0f1117] border border-white/8 rounded-2xl shadow-2xl overflow-hidden animate-scale-in">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/6">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg hover:bg-white/6 text-gray-500 hover:text-white transition-colors"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto max-h-[75vh]">
          ...
          {selectedGroup && (
            <div className="flex items-start gap-2 p-3 rounded-lg bg-indigo-500/5 border border-indigo-500/15 text-xs text-indigo-300/80">
              ...
              <span>
                ...
                <code className="text-indigo-300 font-mono">{selectedGroup.logicalId}</code>. No additional gateway
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-600 mt-1">
              Short label to distinguish this cert within the group (e.g. "primary", "backup-2025")
            </p>
          </div>
              ...
              <Select
                ...
                options={[
                  { value: 'PEM', label: 'PEM', description: 'Privacy-Enhanced Mail — most common' },
                  {
                    value: 'PKCS12',
                    label: 'PKCS12 / PFX',
                    description: 'Binary format, common in enterprise environments',
                  },
                ]}
              />
            </div>
            ...
            <div className="flex items-center justify-between mb-1.5">
              ...
              <label className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-white/4 border border-white/8 text-xs text-gray-400 hover:text-white cursor-pointer transition-colors">
                ...
                <input
                  ...
                  className="hidden"
                  onChange={handleFileRead('certPem')}
                />
              </label>
            </div>
            <textarea
              ...
              <textarea
                value={
                  showPrivateKey
                    ? form.privateKey
                    : form.privateKey
                      ? '•'.repeat(Math.min(form.privateKey.length, 60)) + (form.privateKey.length > 60 ? '…' : '')
                      : ''
                }
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-white/6 bg-[#0c0e14]">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 rounded-lg text-sm text-gray-400 hover:text-white hover:bg-white/4 transition-colors"
          >
            Cancel
          </button>
          <button
