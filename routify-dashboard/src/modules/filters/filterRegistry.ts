...
export const FILTER_REGISTRY: FilterRegistryEntry[] = [
  // ── Authentication ────────────────────────────────────────────────────────────
  {
    value: 'AUTH_JWT',        label: 'JWT Auth',         category: 'Authentication',
    description: 'Validate RS256/HS256 JWT bearer tokens; injects X-Auth-User-Id, X-Auth-Tenant-Id, X-Auth-Role, X-Auth-Email downstream',
    color: 'text-emerald-400', bg: 'bg-emerald-400/10', border: 'border-emerald-400/20',
  },
  {
    value: 'AUTH_API_KEY',    label: 'API Key Auth',     category: 'Authentication',
  ...
  {
    value: 'AUTH_BASIC',      label: 'Basic Auth',       category: 'Authentication',
  ...
  {
    value: 'AUTH_OAUTH2',     label: 'OAuth2',           category: 'Authentication',
  ...
  {
    value: 'AUTH_MTLS',       label: 'mTLS Auth',        category: 'Authentication',
  ...
  {
    value: 'AUTH_CLIENT_ID',  label: 'Client ID Auth',   category: 'Authentication',
  ...
  {
    value: 'AUTH_CERT_VAULT', label: 'Cert Vault Auth',  category: 'Authentication',
    description: 'Authenticate caller by verifying their PEM certificate against a Certificate Group in the Cert Vault registry',
    color: 'text-sky-400', bg: 'bg-sky-400/10', border: 'border-sky-400/20',
  },

  // ── Downstream Auth ───────────────────────────────────────────────────────────
  {
    value: 'DOWNSTREAM_BASIC_AUTH', label: 'Downstream Basic Auth', category: 'Downstream Auth',
    description: 'Inject Basic Auth credentials into outbound downstream requests',
    color: 'text-violet-400', bg: 'bg-violet-400/10', border: 'border-violet-400/20',
  },
  {
    value: 'DOWNSTREAM_BEARER_CC',  label: 'Downstream Bearer (CC)', category: 'Downstream Auth',
  ...
  // ── Rate Limiting ─────────────────────────────────────────────────────────────
  {
    value: 'RATE_LIMIT_FIXED_WINDOW',   label: 'Fixed Window Rate Limit',   category: 'Rate Limiting',
  ...
  {
    value: 'RATE_LIMIT_SLIDING_WINDOW', label: 'Sliding Window Rate Limit', category: 'Rate Limiting',
  ...
  // ── Modification ─────────────────────────────────────────────────────────────
  {
    value: 'REQUEST_HEADER_MODIFY',  label: 'Request Header Modify',  category: 'Modification',
    description: 'Add, set or remove request headers before forwarding upstream',
    color: 'text-blue-300', bg: 'bg-blue-300/10', border: 'border-blue-300/20',
  },
  {
    value: 'RESPONSE_HEADER_MODIFY', label: 'Response Header Modify', category: 'Modification',
    description: 'Add, set or remove response headers after upstream replies',
    color: 'text-purple-300', bg: 'bg-purple-300/10', border: 'border-purple-300/20',
  },

  // ── Transformation ────────────────────────────────────────────────────────────
  {
    value: 'BODY_JOLT_TRANSFORM', label: 'Jolt Transform', category: 'Transformation',
  ...
  // ── Validation ────────────────────────────────────────────────────────────────
  {
    value: 'VALIDATE_JSON_SCHEMA', label: 'JSON Schema Validate', category: 'Validation',
    description: 'Validate request body against a JSON Schema (Draft-07 by default)',
    color: 'text-cyan-300', bg: 'bg-cyan-300/10', border: 'border-cyan-300/20',
  },

  // ── Resilience ────────────────────────────────────────────────────────────────
  {
    value: 'TIMEOUT', label: 'Request Timeout', category: 'Resilience',
  ...
  // ── Observability ─────────────────────────────────────────────────────────────
  {
    value: 'CORRELATION_ID', label: 'Correlation ID',  category: 'Observability',
    description: 'Inject or propagate X-Correlation-Id (generate UUID if absent); runs at order −1000',
    color: 'text-indigo-300', bg: 'bg-indigo-300/10', border: 'border-indigo-300/20',
  },
  {
    value: 'REQUEST_LOGGER', label: 'Request Logger',  category: 'Observability',
    description: 'Log requests/responses and publish telemetry to the routify.request.telemetry Kafka topic',
    color: 'text-gray-300', bg: 'bg-gray-300/10', border: 'border-gray-300/20',
  },
  {
  ...
  {
    value: 'CUSTOM_METRIC',  label: 'Custom Metric',   category: 'Observability',
  ...
  // ── Security ─────────────────────────────────────────────────────────────────
  {
    value: 'SECURITY_HEADERS',        label: 'Security Headers',        category: 'Security',
  ...
  {
    value: 'CERT_ROTATION',           label: 'Cert Rotation',           category: 'Security',
    description: 'Enforce certificate rotation — reject requests with revoked or unknown client certificates',
    color: 'text-sky-400', bg: 'bg-sky-400/10', border: 'border-sky-400/20',
  },
  {
    value: 'CERT_VAULT_EXPIRY_CHECK', label: 'Cert Vault Expiry Check', category: 'Security',
    description: 'Block or warn when all certificates in a Cert Group are expired, revoked, or approaching expiry',
    color: 'text-rose-300', bg: 'bg-rose-300/10', border: 'border-rose-300/20',
  },

  // ── Versioning ────────────────────────────────────────────────────────────────
  {
    value: 'API_VERSIONING', label: 'API Versioning', category: 'Versioning',
  ...
  // ── Routing ───────────────────────────────────────────────────────────────────
  {
    value: 'CONDITIONAL_ROUTE',       label: 'Conditional Route',       category: 'Routing',
    description: 'Rewrite upstream URI when a header or query param matches a pattern',
    color: 'text-pink-400', bg: 'bg-pink-400/10', border: 'border-pink-400/20',
  },
  {
    value: 'USER_ID_PAYLOAD_ROUTING', label: 'User ID Payload Routing', category: 'Routing',
  ...
  // ── Custom ────────────────────────────────────────────────────────────────────
  {
    value: 'CUSTOM_SPEL', label: 'Custom (SpEL)', category: 'Custom',
  ...
  // ── AI ────────────────────────────────────────────────────────────────────────
  {
    value: 'AI_FILTER',   label: 'AI Filter',   category: 'AI',
    description: 'LLM-powered, policy-driven request filtering — ALLOW / BLOCK / FLAG verdicts via natural-language rules',
    color: 'text-fuchsia-400', bg: 'bg-fuchsia-400/10', border: 'border-fuchsia-400/20',
  },
  {
    value: 'AI_MODIFIER', label: 'AI Modifier', category: 'AI',
    description: 'LLM-powered request mutation — PII scrubbing, payload translation, header rewriting before routing downstream',
    color: 'text-pink-500', bg: 'bg-pink-500/10', border: 'border-pink-500/20',
  },
]

/** Keyed map for O(1) lookups by FilterType value */
export const FILTER_REGISTRY_MAP = new Map<FilterType, FilterRegistryEntry>(
  FILTER_REGISTRY.map(e => [e.value, e])
)
