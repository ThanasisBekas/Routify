      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        {data.inProgress > 0 && (
          <span className="flex items-center gap-1.5 text-xs text-blue-400">
            ...
            <strong>{data.inProgress}</strong> in progress
          </span>
        )}
        {data.succeeded > 0 && (
          <span className="flex items-center gap-1.5 text-xs text-emerald-400">
            <CheckCircle2 className="w-3 h-3" />
            <strong>{data.succeeded}</strong> succeeded
          </span>
        )}
        {data.failed > 0 && (
...
function TH({ children }: { children: React.ReactNode }) {
  return (
    <th className="px-4 py-3 text-[11px] font-semibold text-gray-500 uppercase tracking-widest first:px-6">
      {children}
    </th>
  )
}
...
export default function AuditPage() {
  ...
  const liveEvents = liveFeed
    ? recentEvents.filter(
        (e) => !['metrics', 'connected', 'pong', 'gateway.config.changed', 'gateway.reloaded'].includes(e.type),
      )
    : []
  ...
  const pendingCount =
    failedQuery.data?.content?.filter((r) => r.replayStatus === 'PENDING' || r.replayStatus === 'FAILED').length ?? 0
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Immutable event trail, full request telemetry, and failed-request replay
            </p>
          </div>
        ...
        <div className="flex items-center gap-2 px-6 py-2.5 border-b border-white/[0.06] bg-[#0a0c12]">
          ...
          {(['pending', 'all'] as const).map((f) => (
            <button
              ...
              className={cn(
                'text-xs px-3 py-1 rounded-md border font-medium transition-all',
                replayFilter === f
              ...
              <div key={e.id} className="flex items-center gap-3 px-6 py-2 border-t border-emerald-500/5">
                ...
                <span
                  className={cn(
                    ...
                    e.type.includes('activated')
                      ? 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20'
                      : e.type.includes('deactivated')
                        ? 'text-amber-400 bg-amber-400/10 border-amber-400/20'
                        : e.type.includes('deleted')
        ...
        {isLoading ? (
          ...
          events.length === 0 ? (
            <EmptyState
              ...
              desc="Events appear here as actions occur on the platform"
            />
          ) : (
              ...
              <tbody>
                {events.map((e) => (
                  ...
                  >
                    <td className="px-6 py-3">
                      <span
                        className={cn(
                          ...
                          e.eventType.includes('ACTIVATED')
                            ? 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20'
                            : e.eventType.includes('DEACTIVATED')
                              ? 'text-amber-400 bg-amber-400/10 border-amber-400/20'
                              : e.eventType.includes('DELETED')
                                ? 'text-red-400 bg-red-400/10 border-red-400/20'
                                : 'text-indigo-400 bg-indigo-400/10 border-indigo-400/20',
                        )}
                      >
                    ...
                    <td className="px-4 py-3">
                      <code className="text-xs text-gray-500 font-mono bg-white/[0.03] px-1.5 py-0.5 rounded">
                        {e.aggregateId?.slice(0, 8)}…
                      </code>
                    </td>
          ...
          requests.length === 0 ? (
            <EmptyState
              ...
              desc="Request logs appear here once traffic flows through the gateway"
            />
          ) : (
            <table className="w-full text-sm">
              <thead>
              ...
              <tbody>
                {requests.map((r) => (
                  <tr
                    ...
                    className={cn(
                      'border-b border-white/[0.04] hover:bg-white/[0.025] transition-colors cursor-pointer group',
                      r.failed && 'bg-red-500/[0.02]',
                    )}
                  >
                    ...
                    <td className="px-4 py-3">
                      <code className="text-xs text-gray-400 font-mono">{r.path}</code>
                    </td>
                    <td className="px-4 py-3">
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 max-w-[180px]">
                      {r.errorMessage ? (
                        <span className="text-[10px] text-red-400/80 truncate block" title={r.errorMessage}>
                          {r.errorMessage}
                        </span>
                      ) : (
                        <span className="text-[10px] text-gray-600">—</span>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-1">
                        ...
                        {r.replayError && (
                          <span className="text-[10px] text-red-500/70 truncate max-w-[140px]" title={r.replayError}>
                            {r.replayError}
                          </span>
                        )}
                      </div>
      ...
      {replayBulk.isSuccess && replayBulk.data && (
        <div className="mx-6 mb-4 px-4 py-3 rounded-xl border border-indigo-500/20 bg-indigo-500/[0.06] flex items-center gap-3">
          ...
          <span className="text-xs text-indigo-300">
            Bulk replay complete — <strong>{replayBulk.data.succeeded}</strong> succeeded,{' '}
            <strong>{replayBulk.data.failed}</strong> failed, <strong>{replayBulk.data.skipped}</strong> skipped out of{' '}
