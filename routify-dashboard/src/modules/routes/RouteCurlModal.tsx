...
function loadGatewayUrl() {
  try { return localStorage.getItem(GATEWAY_URL_KEY) ?? 'http://localhost:8080' } catch { return 'http://localhost:8080' }
}
function saveGatewayUrl(v: string) {
  try { localStorage.setItem(GATEWAY_URL_KEY, v) } catch { /* ignore */ }
}
...
const AUTH_FILTER_TYPES = new Set([
  'AUTH_JWT', 'AUTH_BASIC', 'AUTH_API_KEY', 'AUTH_OAUTH2', 'AUTH_MTLS', 'AUTH_CLIENT_ID',
])
...
function buildCurl(opts: {
  ...
  // Build URL
  const qs = queryParams.filter(p => p.key).map(p => `${encodeURIComponent(p.key)}=${encodeURIComponent(p.value)}`).join('&')
...
export default function RouteCurlModal({
  route,
  onClose,
}: {
  route: RouteDto
  ...
  const [path, setPath] = useState(
    route.pathPattern.replace(/\*\*$/, '').replace(/\*$/, ''),
  )
  ...
  const [selectedAuthFilterType, setSelectedAuthFilterType] = useState(
    authFilters[0]?.filterType ?? 'AUTH_NONE',
  )
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-b border-white/[0.07] shrink-0">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg text-gray-500 hover:text-white hover:bg-white/[0.05] transition-all">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="px-6 py-5 space-y-5">
            ...
            {/* Gateway URL */}
            <div>
              <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-widest mb-1.5">Gateway URL</label>
              <input
                value={gatewayUrl}
            ...
            <div className="flex gap-3">
              ...
              <div className="shrink-0">
                <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-widest mb-1.5">Method</label>
              ...
              {/* Path */}
              <div className="flex-1 min-w-0">
                <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-widest mb-1.5">Path</label>
                <input
                  value={path}
            ...
            <div className="flex items-center gap-2 px-3 py-2 bg-indigo-500/[0.05] border border-indigo-500/20 rounded-lg">
              <span className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider shrink-0">X-Tenant-Id</span>
              ...
              {authFilters.length === 0 ? (
                ...
                <div className="space-y-4">
                  ...
                  {authFilters.length > 1 && (
                    <div>
                      <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-widest mb-1.5">Auth filter</label>
                      <div className="flex gap-1.5 flex-wrap">
                        {authFilters.map(f => (
                          <button
                            key={f.filterId}
                            onClick={() => { setSelectedAuthFilterType(f.filterType); setAuthValues({}) }}
                            className={cn(
                  ...
                  {authHint.note && (
                    <p className="text-xs text-gray-500 italic">{authHint.note}</p>
                  )}
                </div>
              )}
            </Accordion>
            ...
            {route.filters.length > 0 && (
              <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] px-4 py-3">
                <p className="text-[10px] font-bold text-gray-600 uppercase tracking-widest mb-2">Active filter chain</p>
                <div className="flex flex-wrap gap-1.5">
                  {[...route.filters].sort((a, b) => a.order - b.order).map(f => (
                    <span
                      key={f.filterId}
                      className={cn(
...
function Accordion({
  label, badge, badgeColor = 'gray', open, onToggle, children,
}: {
  label: string
  badge?: string | number
  ...
  return (
    <div className="rounded-xl border border-white/[0.07] overflow-hidden">
      ...
      {open && (
        <div className="px-4 py-4 bg-transparent border-t border-white/[0.05]">
          {children}
        </div>
      )}
    </div>
...
function KVEditor({
  rows, onAdd, onRemove, onUpdate, keyPlaceholder, valuePlaceholder,
}: {
  rows: KV[]
  onAdd: () => void
  ...
  const inputCls = "flex-1 bg-white/[0.04] border border-white/[0.08] rounded-lg px-2.5 py-1.5 text-xs text-white font-mono placeholder-gray-600 focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500/20 transition-all"
  return (
