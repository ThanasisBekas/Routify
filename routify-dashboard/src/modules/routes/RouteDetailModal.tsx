            ...
            {route && (
              <>
                ...
                <button
                  onClick={() => {
                    onClose()
                    navigate(`/routes/${routeId}/builder`)
                  }}
            ...
            <>
              {route.status === 'ACTIVE' && (
                <div className="flex items-center gap-2 p-3 bg-emerald-500/5 border border-emerald-500/20 rounded-lg text-xs text-emerald-400 shrink-0">
                  ...
                  <span>
                    Route is <strong>live</strong>. Filter chain changes apply instantly via Kafka hot-reload.
                  </span>
                </div>
              )}
              ...
              {view === 'config' && (
                <div className="bg-white/[0.03] rounded-xl p-5 space-y-3 text-sm border border-white/[0.05]">
                  <ConfigRow
                    label="Path"
                    value={<code className="font-mono text-indigo-300 text-xs">{route.pathPattern}</code>}
                  />
                  <ConfigRow
                    label="Methods"
                    value={
                      <div className="flex gap-1 flex-wrap">
                        {route.methods.split(',').map((m) => (
                          <span
                            key={m}
                            className="text-[10px] px-1.5 py-0.5 rounded bg-indigo-500/20 text-indigo-300 font-mono font-bold"
                          >
                            {m.trim()}
                          </span>
                        ))}
                  ...
                  <ConfigRow
                    label="Upstream"
                    value={<code className="font-mono text-gray-300 text-xs break-all">{route.upstreamUri}</code>}
                  />
                  {route.stripPrefix && (
                    <ConfigRow
                      label="Strip Prefix"
                      value={<code className="font-mono text-xs text-gray-400">{route.stripPrefix}</code>}
                    />
                  )}
                  {route.description && (
                  ...
                  <ConfigRow
                    label="Filters"
                    value={<span className="text-gray-300">{route.filters?.length ?? 0} attached</span>}
                  />
                  <ConfigRow
                    label="Status"
                    value={<span className="text-gray-300 font-semibold">{route.status}</span>}
                  />
                  <ConfigRow
                    label="Created"
                    value={<span className="text-gray-500 text-xs">{new Date(route.createdAt).toLocaleString()}</span>}
                  />
                  {route.activatedAt && (
                    <ConfigRow
                      label="Activated"
                      value={
                        <span className="text-gray-500 text-xs">{new Date(route.activatedAt).toLocaleString()}</span>
                      }
                    />
                  )}
                </div>
