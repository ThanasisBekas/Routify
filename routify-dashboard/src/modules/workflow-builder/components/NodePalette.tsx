...
export default function NodePalette({ className }: NodePaletteProps) {
  ...
  const onDragStart = useCallback((event: React.DragEvent, node: PaletteNodeDef) => {
...
function DraggableNodeCard({
  ...
  return (
    <div
      ...
      title={
        isLocked ? 'Pause the route to add this node' :
        isUsed   ? `${node.label} is already on the canvas` :
                   node.description
      }
    >
      {/* Drag grip indicator */}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1">
          ...
          {isLocked && (
            <Lock className="w-2.5 h-2.5 text-gray-700 shrink-0" />
          )}
        </div>
