...
import {
  ArrowLeft, Play, Pause, RefreshCw, Save, Zap, Network, Trash2, AlertTriangle, X, Lock,
} from 'lucide-react'
      ...
      <div className="bg-[#111318] border border-white/[0.08] rounded-2xl w-full max-w-sm shadow-2xl animate-fade-in-up">
        ...
        <div className="px-5 py-4 space-y-3">
          ...
          <p className="text-xs text-gray-500 leading-relaxed">
            The route itself won't be deleted — only the current canvas layout will be wiped.
            You can rebuild from scratch or reload from the backend.
          </p>
        </div>
...
export default function WorkflowBuilderPage() {
  ...
  useEffect(() => {
    if (route) {
      ...
      const { nodes, edges } = (route.filters?.length ?? 0) > 0
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06] bg-[#0d0f14] shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={isLocked ? 'Pause the route first to clear the canvas' : 'Remove all nodes and connections from the canvas'}
          >
            <Trash2 className="w-3.5 h-3.5" />
            Clear
          </button>
      ...
      {isLocked && (
        <div className="flex items-center gap-3 px-4 py-2.5 bg-emerald-500/[0.06] border-b border-emerald-500/20 shrink-0">
          ...
          <p className="text-xs text-emerald-300 flex-1">
            <span className="font-semibold">This route is active.</span>{' '}
            Pause it to make changes — adding nodes, removing filters, editing properties, and clearing the canvas are all disabled while live.
          </p>
          <button
            onClick={() => deactivateMutation.mutate()}
            disabled={deactivateMutation.isPending}
